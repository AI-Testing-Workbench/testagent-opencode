...
2. 检查你的 API 密钥是否有效
3. 确保你的网络允许连接到模型API的 API

---

### 模型不可用

1. 检查你是否已通过模型API的身份验证
2. 验证配置中的模型名称是否正确
...
1. 首先，按照[模型API指南](/docs/providers)验证你的模型API是否已正确设置
...
3. 在 TUI 中使用 `/connect` 命令重新与模型API进行身份验证。

---

### AI_APICallError 和模型API包问题

如果你遇到 API 调用错误，可能是由于模型API包过期导致的。OpenCode 会根据需要动态安装模型API包（OpenAI、Anthropic、Google 等）并将它们缓存到本地。
...
2. 重新启动 OpenCode 以重新安装最新的模型API包

这将强制 OpenCode 下载最新版本的模型API包，通常可以解决模型参数和 API 变更带来的兼容性问题。
