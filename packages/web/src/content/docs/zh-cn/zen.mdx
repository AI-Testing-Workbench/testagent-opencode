...
Zen 的工作方式与 OpenCode 中的任何其他模型API相同。你登录 OpenCode Zen 并获取 API 密钥。它是**完全可选的**，即使不用它，你也可以照常使用 OpenCode。

---

## 背景

现在市面上有大量模型，但其中只有少数模型适合作为编码代理使用。此外，大多数模型API的配置方式差异很大，因此你获得的性能和质量也会非常不同。

:::tip
我们测试了一组与 OpenCode 配合良好的精选模型和模型API。
:::
...
2. 然后我们与几家模型API合作，确保这些模型被正确提供。
3. 最后，我们对模型和模型API的组合进行了基准测试，并整理出了一份我们认为值得推荐的列表。
...
OpenCode Zen 的工作方式与 OpenCode 中的任何其他模型API相同。
...
我们所有模型都托管在 US。我们的模型API遵循零保留政策，不会将你的数据用于模型训练，但以下情况除外：
...
当你使用自己的密钥时，tokens 由模型API直接计费，而不是由 Zen 计费。
...
1. 为编码代理**基准测试**最佳模型和模型API。
2. 提供**最高质量**的选项，而不是降低性能或路由到更便宜的模型API。
3. 通过按成本销售来传递任何**降价**；因此唯一的加价只是为了覆盖我们的处理费用。
4. 保持**无锁定**，允许你将它与任何其他编码代理一起使用。同时也始终允许你在 OpenCode 中使用任何其他模型API。
