...
**OpenCode 不会存储您的代码或上下文数据。**所有处理均在本地完成，或通过直接 API 调用发送至您的 AI 模型API。

这意味着，只要您使用的是信任的模型API或内部 AI 网关，就可以安全地使用 OpenCode。
...
该集中式配置可与您的 SSO 模型API集成，确保所有用户仅访问您的内部 AI 网关。

---

### SSO 集成

通过集中式配置，OpenCode 可以与您组织的 SSO 模型API集成进行身份验证。
...
您还可以禁用所有其他 AI 模型API，确保所有请求都经过组织批准的基础设施。
...
是的。OpenCode 不会存储您的代码或上下文数据。所有处理均在本地完成，或通过直接 API 调用发送至您的 AI 模型API。通过集中式配置和 SSO 集成，您的数据将安全地保留在组织的基础设施内。
