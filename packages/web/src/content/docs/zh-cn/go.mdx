...
Go 的工作方式与 OpenCode 中的任何其他模型API（provider）一样。订阅 OpenCode Go 后你将获得 API 密钥。它是 **完全可选** 的，并非使用 OpenCode 所必需的条件。
...
开源模型现在变得非常强大。在编程任务中，它们的性能已接近专有模型。由于许多模型API都可以提供具有竞争力的服务，它们通常要便宜得多。

然而，获得可靠、低延迟的访问可能很困难。各模型API在质量和可用性方面参差不齐。

:::tip
我们测试了一组经过精选且与 OpenCode 配合良好的模型和模型API。
:::

为了解决这个问题，我们做了以下几件事：

1. 我们测试了一组精选的开源模型，并与他们的团队探讨了如何以最佳方式运行它们。
2. 随后我们与一些模型API合作，以确保正确提供这些服务。
3. 最后，我们对模型和模型API的组合进行了基准测试（benchmark），得出了一份我们乐于推荐的列表。
...
OpenCode Go 的工作方式与 OpenCode 中的其他模型API一样。
...
该方案主要面向国际用户，模型托管在 US、EU 和 Singapore，以提供稳定的全球访问。我们的模型API遵循零保留政策，不会将您的数据用于模型训练。
...
4. **无锁定（no lock-in）**，允许你与 OpenCode 一起使用任何其他模型API。
