---
title: 模型API
description: 在 OpenCode 中使用任意 LLM 模型API。
---

import config from "../../../../config.mjs"
export const console = config.console

OpenCode 使用 [AI SDK](https://ai-sdk.dev/) 和 [Models.dev](https://models.dev)，支持 **75+ LLM 模型API**，同时也支持运行本地模型。

要添加模型API，你需要：

1. 使用 `/connect` 命令添加模型API的 API 密钥。
2. 在 OpenCode 配置中设置该模型API。

---

### 凭据

使用 `/connect` 命令添加模型API的 API 密钥后，凭据会存储在
...
你可以通过 OpenCode 配置中的 `provider` 部分来自定义模型API。

---

#### 自定义 Base URL

你可以通过设置 `baseURL` 选项来自定义任何模型API的 Base URL。这在使用代理服务或自定义端点时非常有用。
...
它的使用方式与 OpenCode 中的其他模型API完全相同，且完全可选。

---

## 目录

下面我们来详细了解一些模型API。如果你想将某个模型API添加到列表中，欢迎提交 PR。

:::note
没有看到你想要的模型API？欢迎提交 PR。
:::
...
:::note
对于自定义推理配置文件，请在 key 中使用模型名称和模型API名称，并将 `id` 属性设置为 ARN。这可以确保正确的缓存行为：
...
Cloudflare AI Gateway 允许你通过统一端点访问来自 OpenAI、Anthropic、Workers AI 等模型API的模型。通过 [Unified Billing](https://developers.cloudflare.com/ai-gateway/features/unified-billing/)，你无需为每个模型API单独准备 API 密钥。
...
[Hugging Face Inference Providers](https://huggingface.co/docs/inference-providers) 提供对由 17+ 模型API支持的开放模型的访问。
...
[Helicone](https://helicone.ai) 是一个 LLM 可观测性平台，为你的 AI 应用提供日志记录、监控和分析功能。Helicone AI Gateway 会根据模型自动将请求路由到对应的模型API。
...
如需了解更多模型API以及缓存、速率限制等高级功能，请查阅 [Helicone 文档](https://docs.helicone.ai)。
...
Helicone 支持用于缓存、用户跟踪和会话管理等功能的自定义请求头。使用 `options.headers` 将它们添加到模型API配置中：
...
- `llama.cpp` 是自定义的模型API ID，可以是任意字符串。
- `npm` 指定该模型API使用的包。这里使用 `@ai-sdk/openai-compatible` 来兼容任何 OpenAI 兼容的 API。
- `name` 是该模型API在 UI 中显示的名称。
...
- `lmstudio` 是自定义的模型API ID，可以是任意字符串。
- `npm` 指定该模型API使用的包。这里使用 `@ai-sdk/openai-compatible` 来兼容任何 OpenAI 兼容的 API。
- `name` 是该模型API在 UI 中显示的名称。
- `options.baseURL` 是本地服务器的端点地址。
- `models` 是模型 ID 到其配置的映射。模型名称会显示在模型选择列表中。
...
5. 你还可以通过 OpenCode 配置自定义模型。以下是指定模型API的示例：
...
Vercel AI Gateway 允许你通过统一端点访问来自 OpenAI、Anthropic、Google、xAI 等模型API的模型。模型按原价提供，不额外加价。
...
你也可以通过 OpenCode 配置自定义模型。以下是指定模型API路由顺序的示例。
...
| `order`             | 模型API尝试顺序                   |
| `only`              | 限制为特定模型API                 |
| `zeroDataRetention` | 仅使用具有零数据留存策略的模型API |
...
要添加 `/connect` 命令中未列出的任何 **OpenAI 兼容**模型API：

:::tip
你可以在 OpenCode 中使用任何 OpenAI 兼容的模型API。大多数现代 AI 模型API都提供 OpenAI 兼容的 API。
:::
...
4. 在项目目录中创建或更新 `opencode.json` 文件：
   ...
   以下是配置选项说明：
   - **npm**：要使用的 AI SDK 包，对于 OpenAI 兼容的模型API使用 `@ai-sdk/openai-compatible`（适用于 `/v1/chat/completions`）。如果你的模型API/模型走 `/v1/responses`，请使用 `@ai-sdk/openai`。
   - **name**：在 UI 中显示的名称。
   - **models**：可用模型。
...
5. 执行 `/models` 命令，你自定义的模型API和模型将出现在选择列表中。
...
`limit` 字段让 OpenCode 了解你还剩余多少上下文空间。标准模型API会自动从 models.dev 拉取这些信息。

---

## 故障排除

如果你在配置模型API时遇到问题，请检查以下几点：

1. **检查认证设置**：运行 `opencode auth list` 查看该模型API的凭据是否已添加到配置中。

   这不适用于 Amazon Bedrock 等依赖环境变量进行认证的模型API。

2. 对于自定义模型API，请检查 OpenCode 配置并确认：
   - `/connect` 命令中使用的模型API ID 与 OpenCode 配置中的 ID 一致。
   - 使用了正确的 npm 包。例如，Cerebras 应使用 `@ai-sdk/cerebras`。对于其他所有 OpenAI 兼容的模型API，使用 `@ai-sdk/openai-compatible`（`/v1/chat/completions`）；如果模型走 `/v1/responses`，请使用 `@ai-sdk/openai`。同一 provider 混用时，可在模型下设置 `provider.npm` 覆盖默认值。
