...
组织可以通过 `.well-known/opencode` 端点提供默认配置。当您使用支持该功能的模型API进行身份验证时，会自动获取此配置。
...
将全局 OpenCode 配置放在 `~/.config/opencode/opencode.json` 中。使用全局配置来设置用户级别的偏好，例如主题、模型API或快捷键。
...
您可以通过 `provider`、`model` 和 `small_model` 选项在 OpenCode 配置中设置要使用的模型API和模型。
...
`small_model` 选项为标题生成等轻量级任务配置单独的模型。默认情况下，如果您的模型API有更便宜的模型可用，OpenCode 会尝试使用该模型，否则会回退到您的主模型。

模型API选项可以包括 `timeout` 和 `setCacheKey`：
...
一些模型API支持除通用 `timeout` 和 `apiKey` 设置之外的额外配置选项。
...
您可以通过 `disabled_providers` 选项禁用自动加载的模型API。当您希望阻止某些模型API被加载（即使其凭据可用）时，此选项非常有用。
...
`disabled_providers` 选项接受模型API ID 的数组。当某个模型API被禁用时：

- 即使设置了环境变量，也不会被加载。
- 即使通过 `/connect` 命令配置了 API 密钥，也不会被加载。
- 该模型API的模型不会出现在模型选择列表中。

---

### 启用模型API

您可以通过 `enabled_providers` 选项指定允许使用的模型API白名单。设置后，仅启用指定的模型API，所有其他模型API将被忽略。
...
当您希望限制 OpenCode 仅使用特定模型API，而不是逐一禁用其他模型API时，此选项非常有用。

:::note
`disabled_providers` 优先于 `enabled_providers`。
:::

如果某个模型API同时出现在 `enabled_providers` 和 `disabled_providers` 中，为了向后兼容，`disabled_providers` 优先。
