...
OpenCode 基于 [Models.dev](https://models.dev) 的模型API列表运行，因此您可以使用 `opencode auth login` 为任何想要使用的模型API配置 API 密钥。密钥存储在 `~/.local/share/opencode/auth.json` 中。

```bash
opencode auth login
```

OpenCode 启动时会从凭据文件加载模型API信息，同时也会加载环境变量或项目中 `.env` 文件中定义的密钥。

---

#### list

列出凭据文件中存储的所有已认证模型API。
...
此命令以 `provider/model` 的格式显示所有已配置模型API中可用的模型。

这对于确定在[配置文件](/docs/config/)中使用的确切模型名称非常有用。

您可以选择传入模型API ID 来按模型API筛选模型。
...
使用 `--refresh` 标志可以更新缓存的模型列表。当模型API新增了模型并且您希望在 OpenCode 中看到它们时，此功能非常有用。
...
| `OPENCODE_FAKE_VCS`                   | string  | 用于测试目的的模拟 VCS 模型API           |
