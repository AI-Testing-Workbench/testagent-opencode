...
OpenCode 使用 [AI SDK](https://ai-sdk.dev/) 和 [Models.dev](https://models.dev)，支援 **75+ LLM 模型API**，同時也支援執行本地模型。

要新增模型API，您需要：

1. 使用 `/connect` 指令新增模型API的 API 金鑰。
2. 在 OpenCode 設定中設定該模型API。

---

### 憑證

使用 `/connect` 指令新增模型API的 API 金鑰後，憑證會儲存在
...
您可以透過 OpenCode 設定中的 `provider` 部分來自訂模型API。

---

#### 自訂 Base URL

您可以透過設定 `baseURL` 選項來自訂任何模型API的 Base URL。這在使用代理服務或自訂端點時非常有用。
...
它的使用方式與 OpenCode 中的其他模型API完全相同，且完全可選。
...
它的使用方式與 OpenCode 中的任何其他模型API相同，且完全可選。

---

## 目錄

下面我們來詳細了解一些模型API。如果您想將某個模型API新增到列表中，歡迎提交 PR。

:::note
沒有看到您想要的模型API？歡迎提交 PR。
:::
...
:::note
對於自訂推理設定檔，請在 key 中使用模型名稱和模型API名稱，並將 `id` 屬性設定為 ARN。這可以確保正確的快取行為：
...
Cloudflare AI Gateway 允許您透過統一端點存取來自 OpenAI、Anthropic、Workers AI 等模型API的模型。透過 [Unified Billing](https://developers.cloudflare.com/ai-gateway/features/unified-billing/)，您無需為每個模型API單獨準備 API 金鑰。
...
[Hugging Face Inference Providers](https://huggingface.co/docs/inference-providers) 提供對由 17+ 模型API支援的開放模型的存取。
...
[Helicone](https://helicone.ai) 是一個 LLM 可觀測性平台，為您的 AI 應用程式提供日誌記錄、監控和分析功能。Helicone AI Gateway 會根據模型自動將請求路由到對應的模型API。
...
如需了解更多模型API以及快取、速率限制等進階功能，請查閱 [Helicone 文件](https://docs.helicone.ai)。
...
Helicone 支援用於快取、使用者追蹤和工作階段管理等功能的自訂請求標頭。使用 `options.headers` 將它們新增到模型API設定中：
...
- `llama.cpp` 是自訂的模型API ID，可以是任意字串。
- `npm` 指定該模型API使用的套件。這裡使用 `@ai-sdk/openai-compatible` 來相容任何 OpenAI 相容的 API。
- `name` 是該模型API在 UI 中顯示的名稱。
...
5. 您還可以透過 OpenCode 設定自訂模型。以下是指定模型API的範例：
...
Vercel AI Gateway 允許您透過統一端點存取來自 OpenAI、Anthropic、Google、xAI 等模型API的模型。模型按原價提供，不額外加價。
...
您也可以透過 OpenCode 設定自訂模型。以下是指定模型API路由順序的範例。
...
| `order`             | 模型API嘗試順序                   |
| `only`              | 限制為特定模型API                 |
| `zeroDataRetention` | 僅使用具有零資料留存策略的模型API |
...
要新增 `/connect` 指令中未列出的任何 **OpenAI 相容**模型API：

:::tip
您可以在 OpenCode 中使用任何 OpenAI 相容的模型API。大多數現代 AI 模型API都提供 OpenAI 相容的 API。
:::
...
4. 在專案目錄中建立或更新 `opencode.json` 檔案：
   ...
   以下是設定選項說明：
   - **npm**：要使用的 AI SDK 套件，對於 OpenAI 相容的模型API使用 `@ai-sdk/openai-compatible`
   - **name**：在 UI 中顯示的名稱。
   - **models**：可用模型。
...
5. 執行 `/models` 指令，您自訂的模型API和模型將出現在選擇列表中。
...
`limit` 欄位讓 OpenCode 了解您還剩餘多少上下文空間。標準模型API會自動從 models.dev 拉取這些資訊。

---

## 疑難排解

如果您在設定模型API時遇到問題，請檢查以下幾點：

1. **檢查認證設定**：執行 `opencode auth list` 查看該模型API的憑證是否已新增到設定中。

   這不適用於 Amazon Bedrock 等依賴環境變數進行認證的模型API。

2. 對於自訂模型API，請檢查 OpenCode 設定並確認：
   - `/connect` 指令中使用的模型API ID 與 OpenCode 設定中的 ID 一致。
   - 使用了正確的 npm 套件。例如，Cerebras 應使用 `@ai-sdk/cerebras`。對於其他所有 OpenAI 相容的模型API，使用 `@ai-sdk/openai-compatible`。
