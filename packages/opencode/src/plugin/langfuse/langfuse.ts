...
const allToolDefs = new Map<
  string,
  { id: string; description: string; parameters: any }
>();

// 存储 LLM 输出数据
const llmOutputs = new Map<
  string,
  { text: string; tool_calls: any[]; usage: any; reasoning: string }
>();
...
function sanitize(input: any): any {
  ...
  for (const [k, v] of Object.entries(input)) {
    // 对包含敏感关键词的字段进行脱敏
    out[k] = /^(key|secret|password|token)$/i.test(k)
      ? "[REDACTED]"
      : sanitize(v);
  }
  return out;
...
function extractJsonSchema(obj: Record<string, any>): any {
  ...
  for (const [key, val] of Object.entries(obj)) {
    if (jsonSchemaKeys.has(key)) {
      result[key] =
        typeof val === "object" && val !== null ? extractJsonSchema(val) : val;
    }
  }
  return Object.keys(result).length > 0 ? result : obj;
}
...
export const LangfusePlugin: Plugin = async (ctx) => {
  ...
  return {
    ...
    "chat.message": async (input, output) => {
      ...
      // 创建新的 Trace
      const trace = createNewTrace(
        sessionId,
        textContent || input.message?.content || "message",
        ctx,
        traceId,
      );
    ...
    "chat.params": async (input, output) => {
      ...
      const pastToolCalls =
        input?.message?.metadata?.PasttoolCalls ??
        input?.metadata?.PasttoolCalls ??
        [];
      if (Array.isArray(pastToolCalls) && pastToolCalls.length > 0) {
        const hasSkillCall = pastToolCalls.some(
          (tc: any) => tc?.name === "skill" || tc?.tool === "skill",
        );
      ...
      // 构建模型名称
      const providerId =
        input.provider?.info?.id || input.provider?.id || "unknown";
      ...
      if (output.temperature !== undefined)
        modelParameters.temperature = output.temperature;
      ...
      if (output.maxOutputTokens !== undefined)
        modelParameters.max_tokens = output.maxOutputTokens;
      ...
      if (skillContext) {
        ...
        // 根据是否有父级节点，决定创建方式
        gen = currentParent
          ? currentParent.generation(genParams)
    ...
    "experimental.chat.system.transform": async (input, output) => {
      const sessionId =
        input.sessionID || currentSessionId || [...trackedSessionIds].pop();
    ...
    "tool.execute.before": async (input, output) => {
      ...
      if (!trace && langfuse) {
        trace = createNewTrace(
          sessionId,
          userInputs.get(sessionId) || "tool execution",
          ctx,
          traceId,
        );
      }
      ...
      if (trace) {
        ...
        const spanObj = currentParent
          ? currentParent.span(spanParams)
          : trace.span(spanParams);
      ...
      if (span) {
        ...
        span.end({
          output:
            output.output === null
              ? null
              : String(output.output).slice(0, 10000),
          level,
          metadata: {
            spanKind: "tool",
            tags: OBSERVATION_TAGS,
    ...
    event: async (input: any) => {
      ...
      if (evt.type === "message.part.updated" && evt.properties?.part) {
        ...
        if (g && part.type !== "step-finish") {
          ...
          if (part.type === "tool" && part.state?.status === "running") {
            ...
            if (
              !g.toolCalls.some((tc) => tc.toolCallId === (part.callID || ""))
            ) {
              g.toolCalls.push({
          ...
          if (part.type === "tool-result") {
            g.parts.push(
              `Tool Result: ${part.output?.substring(0, 1000) || ""}`,
            );
          }
        }
        ...
        if (part.type === "step-finish" && part.tokens && g) {
          ...
          const textContent = g.parts
            .filter(
              (p) =>
                !p.startsWith("Tool Call:") &&
                !p.startsWith("Tool Result:") &&
          ...
          const toolCallsOutput = g.toolCalls.map((tc) => ({
            ...
            id:
              tc.toolCallId ||
              `call_${Math.random().toString(36).substring(2, 12)}`,
            name: tc.name,
            input: tc.args || {},
          }));
          ...
          const structuredOutput = {
            text: fullText,
            tool_calls:
              toolCallsOutput.length > 0 ? toolCallsOutput : undefined,
            usage: {
      ...
      if (evt.type === "session.idle") {
        ...
        const sessionId =
          idleSessionId || currentSessionId || [...trackedSessionIds].pop();
        if (!sessionId) return;
