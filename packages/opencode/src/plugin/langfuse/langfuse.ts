      ...
      if (evt.type === "message.part.updated" && evt.properties?.part) {
          ...
          for (let i = allGenerations.length - 1; i >= 0; i--) {
            ...
            if (!g.hasUsage) {
              ...
              const timeToFirstTokenSec = g.completionStartTime
                ? (g.completionStartTime.getTime() - g.startTime.getTime()) /
                  1000
                : null;
              ...
              g.gen.update({
                ...
                output:
                  finalOutput ||
                  `Step ${g.stepNumber} completed with ${part.tokens.total} tokens, cost ${part.cost}`,
                usage: {
        ...
        if (allGenerations.length > 0) {
          ...
          if (part.type === "tool-call") {
            currentGen.parts.push(
              `Tool Call: ${part.name}(${JSON.stringify(part.args)?.substring(0, 500)})`,
            );
          }
